...
/**
 * Pure lane layout for a git log in *newest-first* order (the order `git log
 * --all --topo-order` produces), mirroring `git log --graph`:
 *
 *  - each commit claims the lane waiting for its hash; failing that, the lane
 *    its FIRST parent flows through (so merge commits stay on the main line)
 *    — except for a dangling (unmerged) ref head, which FORKS into its own
 *    lane from the shared parent; failing that, a freed lane, else a new lane;
 *  - the first parent continues the same lane downward;
 *  - every extra parent (merge) reuses the lane already waiting for it, else
 *    weaves into a freed lane (never blindly growing right), else opens a new
 *    lane, and gets a horizontal connector to the node;
 *  - when a commit appears while several lanes wait for it, the leftmost lane
 *    claims it and the others merge into the node.
 *
 * @param commits - newest-first log entries (must carry refs for the fork rule).
 * @param currentBranch - the checked-out branch name (marks the current head;
 *   its head commit still continues the parent lane instead of forking).
 *
 * Rows come back in the same order as the input, so row `i` renders
 * `commits[i]` directly.
 */
